...
handleSet.attachNewData = function(req, next) {
	var newData = replaceServerTimestamp(req.message.data, req.clock);

	if (req.message.isPriorityPath) {
		return req.ref(req.message.path).once('value', function (parentSnap) {
...
function handleSet(req, next) {
	debug('Client set ' + req.message.fullPath);
	var path = req.message.path;
