...
function tryRead(req, next) {
	var result = req.server.tryRead(req.message.path, req.ruleSnapshot, req.authData);
	if (!result.allowed) {
...
function tryWrite(req, next) {
	var path = req.message.path;
	var result = req.server.tryWrite(path, req.ruleSnapshot, req.newData, req.authData);
	if (!result.allowed) {
		req.permissionDenied();
		debug('Permission denied for client to write to %j: %j', path, result.info);
		return;
	}
	next();
}
...
function handleAuth(req, next) {
	var credential = req.message.credentials;
	try {
		var decoded = req.tokenValidator.decodeToken(credential);
		req.authToken = credential;
