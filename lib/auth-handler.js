...
function authData(connection, server) {
	...
	if (connection.authToken) {
		try {
			data = server.decodeToken(connection.authToken).d;
		} catch (e) {
...
function tryRead(message, connection, server, next) {
	var result = server.tryRead(message.path, message.ruleSnapshot, authData(connection, server));
	if (!result.allowed) {
...
function tryWrite(message, connection, server, next) {
	var result = server.tryWrite(message.path, message.ruleSnapshot, message.newData, authData(connection, server));
	if (!result.allowed) {
		connection.permissionDenied(message.requestId);
